defmodule Mix.Tasks.FireTest do
      ...
      {:ok, total_fires} ->
        ...
        if total_count > 0 do
          ...
          Enum.each(recent, fn fire ->
            Mix.shell().info("    #{fire.satellite}: #{fire.latitude}, #{fire.longitude} (#{fire.frp} MW)")
          end)
        end
  ...
  defp test_fetch_satellite_data(satellite, days_back) do
    ...
    url = "https://firms.modaps.eosdis.nasa.gov/api/area/csv/#{api_key}/#{satellite}/world/#{days_back}"
  ...
  defp test_process_csv_data(csv_body, _satellite) do
    ...
    case lines do
      [header | data_lines] when data_lines != [] ->
        ...
        Mix.shell().info("   🎯 #{total_count} total, #{quality_count} quality, #{filtered_out} filtered")
