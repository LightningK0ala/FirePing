defmodule App.Workers.FireFetch do
  ...
  @viirs_satellites [
    "VIIRS_SNPP_NRT",    # Suomi NPP (2011)
    "VIIRS_NOAA20_NRT",  # NOAA-20/JPSS-1 (2017)
    "VIIRS_NOAA21_NRT"   # NOAA-21/JPSS-2 (2022)
  ]
  ...
  def perform(%Oban.Job{} = job) do
    ...
    case fetch_viirs_fire_data(days_back) do
      {:ok, {total_fires, satellite_stats}} ->
        ...
        Logger.info("FireFetch: Successfully processed #{total_fires} fire records from VIIRS constellation", 
  ...
  defp fetch_satellite_data(satellite, days_back) do
    ...
    Logger.info("FireFetch: Fetching #{satellite} data", satellite: satellite, days_back: days_back)
    ...
    case HTTPoison.get(url, headers, timeout: @timeout, recv_timeout: @timeout) do
      ...
      {:error, reason} ->
        log_level = if Mix.env() == :test, do: :debug, else: :error
        Logger.log(log_level, "FireFetch: Network error for #{satellite}", reason: inspect(reason))
  ...
  defp process_csv_data(csv_body, satellite) do
    ...
    case parse_csv(csv_body) do
      {:ok, nasa_data_list} ->
        ...
        _frp_stats = if total_count > 0 do
          frp_values = nasa_data_list 
          ...
          Logger.info("FireFetch: #{satellite} FRP range - min: #{min_frp}, median: #{median_frp}, max: #{max_frp} MW")
        ...
        case Fire.bulk_insert(nasa_data_list) do
          ...
          error ->
            log_level = if Mix.env() == :test, do: :debug, else: :error
            Logger.log(log_level, "FireFetch: Database error for #{satellite}", error: inspect(error))
